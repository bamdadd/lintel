serve: ## Start dev server on :8000 (in-memory storage)
	LINTEL_STORAGE_BACKEND=memory uv run uvicorn lintel.api.app:app --reload --host 0.0.0.0 --port 8000 --reload-exclude '.claude/worktrees/*' --reload-exclude 'worktrees/*' --reload-exclude '.venv/*'
...
serve-db: db-up migrate ## Start dev server with PostgreSQL storage
	LINTEL_STORAGE_BACKEND=postgres LINTEL_DB_DSN=postgresql://lintel:lintel@localhost:5432/lintel LITELLM_LOG=DEBUG uv run uvicorn lintel.api.app:app --reload --host 0.0.0.0 --port 8000 --timeout-graceful-shutdown 2 --reload-exclude '.claude/worktrees/*' --reload-exclude 'worktrees/*' --reload-exclude '.venv/*'
