...
class TriageIssueNode(WorkflowNode):
    ...
    async def execute(
        ...
        await self.tracker.append_log("triage", "Classifying issue...")
        ...
        await self.tracker.append_log("triage", f"Type: {issue_type}, Priority: {priority}")
        await self.tracker.append_log("triage", f"Summary: {summary}")
        ...
        await self.complete(outputs=stage_outputs)
...
# Backward-compatible callable — existing imports keep working.
triage_issue = TriageIssueNode()
