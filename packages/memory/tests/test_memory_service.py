...
@pytest.fixture
def service(mock_repository, mock_vector_store, mock_embedding_service):
...
def _make_fact(
    ...
    **kwargs,
    ...
    return MemoryFact(
        ...
        fact_type=kwargs.get("fact_type", "test_fact"),
        content=kwargs.get("content", "test content"),
        embedding_id=kwargs.get("embedding_id", "emb-1"),
        source_workflow_id=kwargs.get("source_workflow_id"),
    )
...
class TestStoreMemory:
    async def test_generates_embedding_stores_and_saves(
        self, service, mock_embedding_service, mock_vector_store, mock_repository
    ):
        project_id = uuid4()
    ...
    async def test_stores_episodic_in_correct_collection(self, service, mock_vector_store):
    ...
    async def test_passes_source_workflow_id(self, service, mock_repository):
        wf_id = uuid4()
        result = await service.store_memory(
            project_id=uuid4(),
            content="test",
...
class TestRecall:
    async def test_with_specific_memory_type(
        self, service, mock_vector_store, mock_repository, mock_embedding_service
    ):
        project_id = uuid4()
    ...
    async def test_returns_empty_list_when_no_results(self, service, mock_vector_store):
        mock_vector_store.search.return_value = []
    ...
    async def test_ranks_by_descending_score(self, service, mock_vector_store, mock_repository):
        project_id = uuid4()
    ...
    async def test_skips_points_without_fact_id(self, service, mock_vector_store, mock_repository):
        mock_vector_store.search.return_value = [
    ...
    async def test_skips_missing_facts(self, service, mock_vector_store, mock_repository):
        mock_vector_store.search.return_value = [
            ScoredPoint(
                id="emb-1",
                score=0.9,
...
class TestConsolidateFromWorkflow:
    ...
    async def test_stores_new_when_similarity_below_threshold(
        self, service, mock_vector_store, mock_repository
    ):
        project_id = uuid4()
...
class TestDeleteMemory:
    async def test_removes_from_both_stores(self, service, mock_repository, mock_vector_store):
        memory_id = uuid4()
        fact = _make_fact(
            memory_type=MemoryType.LONG_TERM,
    ...
    async def test_returns_false_for_nonexistent(self, service, mock_repository, mock_vector_store):
        mock_repository.get.return_value = None
    ...
    async def test_skips_vector_delete_without_embedding_id(
        self, service, mock_repository, mock_vector_store
    ):
        memory_id = uuid4()
...
class TestInitialize:
    async def test_creates_all_collections(self, service, mock_vector_store):
    ...
    async def test_uses_correct_vector_size(self, service, mock_vector_store):
        await service.initialize()
