...
strategies, KPIs, experiments, compliance metrics, knowledge entries,
and knowledge extraction runs.
"""
...
async def start_knowledge_extraction(
    ...
    await dispatch_event(
        request,
        KnowledgeExtractionStarted(payload={"resource_id": body.run_id, "project_id": body.project_id}),
...
async def add_regulation_from_template(
    ...
    await dispatch_event(
        request,
        RegulationCreated(payload={"resource_id": new_id, "name": template.name, "template_id": body.template_id}),
