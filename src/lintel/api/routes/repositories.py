...
from lintel.contracts.events import RepositoryRegistered, RepositoryUpdated, RepositoryRemoved
...
async def register_repository(
    ...
    await dispatch_event(request, RepositoryRegistered(payload={"resource_id": body.repo_id, "name": body.name, "url": body.url}), stream_id=f"repository:{body.repo_id}")
    return asdict(repo)
...
async def update_repository(
    ...
    await dispatch_event(request, RepositoryUpdated(payload={"resource_id": repo_id, "fields": list(updates.keys())}), stream_id=f"repository:{repo_id}")
...
async def remove_repository(
    ...
    await dispatch_event(request, RepositoryRemoved(payload={"resource_id": repo_id, "name": repo.name}), stream_id=f"repository:{repo_id}")
