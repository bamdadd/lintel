...
from lintel.contracts.events import WorkItemCreated, WorkItemUpdated, WorkItemRemoved
...
async def create_work_item(
    ...
    await dispatch_event(request, WorkItemCreated(payload={"resource_id": body.work_item_id, "title": body.title, "project_id": body.project_id}), stream_id=f"work_item:{body.work_item_id}")
...
async def update_work_item(
    ...
    await dispatch_event(request, WorkItemUpdated(payload={"resource_id": work_item_id, "fields": list(updates.keys())}), stream_id=f"work_item:{work_item_id}")
    return merged
...
async def remove_work_item(
    ...
    await dispatch_event(request, WorkItemRemoved(payload={"resource_id": work_item_id, "title": item.get("title", "")}), stream_id=f"work_item:{work_item_id}")
