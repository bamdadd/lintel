...
async def grant_approval(request: Request, body: GrantApprovalRequest) -> dict[str, Any]:
    ...
    await dispatch_event(request, HumanApprovalGranted(payload={"resource_id": body.thread_ts}), stream_id="approvals")
...
async def reject_approval(request: Request, body: RejectApprovalRequest) -> dict[str, Any]:
    ...
    await dispatch_event(request, HumanApprovalRejected(payload={"resource_id": body.thread_ts}), stream_id="approvals")
    return asdict(command)
