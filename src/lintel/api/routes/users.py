...
async def create_user(
    ...
    await dispatch_event(request, UserCreated(payload={"resource_id": body.user_id, "name": body.name}), stream_id=f"user:{body.user_id}")
...
async def update_user(
    ...
    await dispatch_event(request, UserUpdated(payload={"resource_id": user_id, "fields": list(updates.keys())}), stream_id=f"user:{user_id}")
...
async def delete_user(
    ...
    await dispatch_event(request, UserRemoved(payload={"resource_id": user_id, "name": user.name}), stream_id=f"user:{user_id}")
