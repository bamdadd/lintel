  ...
  return (
    <Stack gap="md">
      ...
      {users.length === 0 ? (
        <EmptyState
          title="No users"
          description="Add users to assign them to teams and projects"
          ...
          <Table.Tbody>
            {users.map((u) => (
              <Table.Tr key={u.user_id}>
                ...
                <Table.Td>
                  <ActionIcon color="red" variant="subtle" onClick={() => handleDelete(u.user_id)}>
                    <IconTrash size={16} />
                  </ActionIcon>
                </Table.Td>
              </Table.Tr>
